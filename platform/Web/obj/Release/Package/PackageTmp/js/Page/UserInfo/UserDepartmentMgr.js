...
function loadnoRoles(userId) {
    ...
    getNoDepartmentByUserID(userId);
}
function loadinRoles(userId) {
    ...
    $('#inr').datagrid({
        url: '../Handler/UserRoleHandler.ashx?visit=loadindepartments&UserID=' + userId,
...
function leftRole() {
    ...
    $.each(row1s, function (index, row1) {
        RoleIDList.push(row1.ID);
    })
    $.post('../Handler/UserRoleHandler.ashx?visit=adddepartments', { UserIDList: JSON.stringify(UserIDList), RoleIDList: JSON.stringify(RoleIDList) }, function (result) {
        if (isDoMore) {
...
function rightRole() {
    ...
    $.post('../Handler/UserRoleHandler.ashx?visit=removedepartments', { UserIDList: JSON.stringify(UserIDList), RoleIDList: JSON.stringify(RoleIDList) }, function (result) {
        if (isDoMore) {
            show_message('操作成功', 'success');
            return;
        }
...
function getNoDepartmentByUserID(userId) {
    var keywords = $('#tdDepartmentName').searchbox('getValue');
    $('#notr').datagrid({
        url: '../Handler/UserRoleHandler.ashx?visit=loadnodepartments&UserID=' + userId + '&keywords=' + keywords,
        onLoadError: function () {
            $("#loadNoRoleFailed").html("<span> 重新加载 </span><a href='javascript:getNoDepartmentByUserID('" + userId + "')'>点击</a>");
        },
        onLoadSuccess: function (data) {
...
function doSearchDepartment() {
    getNoDepartmentByUserID(0);
}
