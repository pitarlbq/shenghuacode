...
function loadGrid(obj) {
    ...
    $.each(obj.list, function (index, listvalue) {
        ...
        var column_title = listvalue.Name;
        column_title = (column_title == '' ? '未定义数量' : '每一' + column_title + '数量');
        columns.push({ field: 'level' + (index + 1), editor: { type: 'numberbox' }, title: column_title, width: 100 });
...
function openytwin() {
    ...
    //}
    open_parent_win('添加业态', 'win_pro', iframeurl, ($(parent.window).width() - 300), $(parent.window).height(), 0, true, true, function () {
