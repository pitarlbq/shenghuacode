...
function TV_Initialize() {
    ...
    DevInfo(0, QNV_DEVINFO_GETCHANNELS, function (nResult) {
        if (nResult <= 0) {
            OpenDevice(0, function (nResult2) {
                DevInfo(0, QNV_DEVINFO_GETCHANNELS, function (channels) {
                    if (channels > 0) {  	//初始化变量
                        ...
                        top.isPhoneDeviceWork = 1;
                    } else {
                        top.isPhoneDeviceWork = 2;
                        top.deviceOnAlert();
...
function TV_StartRecordFile(uID, vFilePath) {
    //vFilePath ="F:\\test.wav";
    if (vFilePath.length > 0) {
        TV_StopPlayFileEx(uID, function () {
            var vFormatID = BRI_WAV_FORMAT_DEFAULT;//BRI_WAV_FORMAT_IMAADPCM8K4B;//选择使用4K/S的ADPCM格式录音
