...
function getColumns() {
    ...
    if (CompanyID < 0 && ProjectIDList.length <= 0 && ServiceTypeID2 < 0 && ServiceTypeID3 < 0) {
        ...
        columns.push({ field: 'ServiceTypeName2', title: '维修类型（二级）', width: 150 });
    }
    if (CompanyID >= 0) {
        columns.push({ field: 'CompanyName', title: '公司', width: 100 });
    }
    ...
    if (ServiceTypeID3 >= 0) {
        columns.push({ field: 'ServiceTypeName3', title: '维修类型（三级）', width: 150 });
    }
    columns.push({ field: 'TotalCount', title: '数量', width: 100 });
